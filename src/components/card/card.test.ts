...
import type OneXCard from './card';
...
describe('<onex-card>', () => {
  ...
  describe('when provided no parameters', () => {
    before(async () => {
      el = await fixture<OnexCard>(
        html` <onex-card>This is just a basic card. No image, no header, and no footer. Just your content.</onex-card> `
      );
    });
