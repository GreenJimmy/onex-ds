...
import type OneXDropdown from './dropdown';

describe('<onex-dropdown>', () => {
  it('should be visible with the open attribute', async () => {
    const el = await fixture<OnexDropdown>(html`
      <onex-dropdown open>
        <onex-button slot="trigger" caret>Toggle</onex-button>
        <onex-menu>
  ...
  it('should still open on arrow navigation when no menu items', async () => {
    ...
    `);
    const trigger = el.querySelector('onex-button')!;
  ...
  it('should close and stop propagating the keydown event when Escape is pressed and the dropdown is open ', async () => {
    ...
    `);
    const firstMenuItem = el.querySelector('onex-menu-item')!;
    const hideHandler = sinon.spy();
