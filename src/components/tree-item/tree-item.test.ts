...
import type OneXTreeItem from './tree-item';

describe('<onex-tree-item>', () => {
  let leafItem: OneXTreeItem;
  let parentItem: OneXTreeItem;

  beforeEach(async () => {
    leafItem = await fixture(html` <onex-tree-item>Node 1</onex-tree-item> `);
  ...
  describe('when the user clicks the expand button', () => {
    describe('and the item is collapsed', () => {
      it('should emit onex-expand and onex-after-expand events', async () => {
        // Arrange
        const expandSpy = sinon.spy();
        const afterExpandSpy = sinon.spy();

        parentItem.addEventListener('onex-expand', expandSpy);
        parentItem.addEventListener('onex-after-expand', afterExpandSpy);
    ...
    describe('and the item is expanded', () => {
      it('should emit onex-collapse and onex-after-collapse events', async () => {
        // Arrange
        const collapseSpy = sinon.spy();
        const afterCollapseSpy = sinon.spy();

        parentItem.addEventListener('onex-collapse', collapseSpy);
        parentItem.addEventListener('onex-after-collapse', afterCollapseSpy);

        parentItem.expanded = true;
        await oneEvent(parentItem, 'onex-after-expand');
  ...
  describe('when the item is lazy', () => {
    it('should emit onex-lazy-change when the lazy attribute is added and removed', async () => {
      // Arrange
      const lazyChangeSpy = sinon.spy();

      parentItem.addEventListener('onex-lazy-change', lazyChangeSpy);
