...
import type OneXImageComparer from './image-comparer';

describe('<onex-image-comparer>', () => {
  it('should render a basic before/after', async () => {
    const el = await fixture<OnexImageComparer>(html`
      <onex-image-comparer>
        <div slot="before"></div>
        <div slot="after"></div>
      </onex-image-comparer>
  ...
  it('should emit change event when position changed manually', async () => {
    ...
    el.addEventListener('onex-change', handler, { once: true });
  ...
  it('should set position by attribute', async () => {
    const el = await fixture<OnexImageComparer>(html`
      <onex-image-comparer position="10">
        <div slot="before"></div>
        <div slot="after"></div>
      </onex-image-comparer>
    `);
