...
- Fixed the border radius in `<onex-dropdown>` so it matches with nested `<onex-menu>` elements
...
- Added the ability to cancel `onex-show` and `onex-hide` events in `<onex-details>` [#993](https://github.com/shoelace-style/shoelace/issues/993)
...
- Fixed a bug in `<onex-tree>` that prevented the keyboard from working when the component was nested in a shadow root [#871](https://github.com/shoelace-style/shoelace/issues/871)
...
- 🚨 BREAKING: improved accessibility of `<onex-radio-group>`, `<onex-radio>`, and `<onex-radio-button>` so they announce properly in screen readers
  ...
  - Removed `setCustomValidity()` and `reportValidity()` from `<onex-radio>` and `<onex-radio-button>` (now available on the radio group)
...
- Added `tag-base`, `tag-content`, and `tag-remove-button` parts to `<onex-select>` [#682](https://github.com/shoelace-style/shoelace/discussions/682)
...
- 🚨 BREAKING: changed the `type` attribute to `variant` in `<onex-alert>`, `<onex-badge>`, `<onex-button>`, and `<onex-tag>` since it's more appropriate and to disambiguate from other `type` attributes
...
- Refactored thumb position logic in `<onex-switch>` [#490](https://github.com/shoelace-style/shoelace/pull/490)
...
- Added experimental `<onex-qr-code>` component
...
- Fixed a bug in `<onex-icon>` where `onex-error` would only be emitted for network failures [#326](https://github.com/shoelace-style/shoelace/pull/326)
...
- 🚨 BREAKING: Refactored `<onex-menu>` and `<onex-menu-item>` to improve accessibility by using proper focus states [#217](https://github.com/shoelace-style/shoelace/issues/217)
...
- Added `name` and `invalid` attribute to `<onex-color-picker>`
...
- Added `required` to `<onex-checkbox>`, `<onex-select>`, and `<onex-switch>`
- Added `reportValidity()` and `setCustomValidity()` methods to all form controls
- Added validation checking for custom and native form controls to `<onex-form>`
- Added `novalidate` attribute to `<onex-form>` to disable validation
...
- Added `hoist` attribute to `<onex-color-picker>`, `<onex-dropdown>`, and `<onex-select>` to work around panel clipping
- Added `<onex-format-bytes>` utility component
- Added `clearable` and `required` props to `<onex-select>`
- Added `slclear` event to `<onex-input>`
- Added keyboard support to the preview resizer in the docs
- Fixed a bug where the `aria-selected` state was incorrect in `<onex-menu-item>`
- Fixed a bug where custom properties applied to `<onex-tooltip>` didn't affect show/hide transitions
- Fixed a bug where `--onex-input-color-*` custom properties had no effect on `<onex-input>` and `<onex-textarea>`
- Refactored `<onex-dropdown>` and `<onex-tooltip>` to use positioner elements so panels/tooltips can be customized easier

## 2.0.0-beta.15

- Added `image-comparer` component
- Added `--width`, `--height`, and `--thumb-size` custom props to `<onex-switch>`
- Fixed an `aria-labelledby` attribute typo in a number of components
- Fixed a bug where the `change` event wasn't updating the value in `<onex-input>`
- Fixed a bug where `<onex-color-picker>` had the wrong border color in the dark theme
- Fixed a bug where `<onex-menu-item>` had the wrong color in dark mode when disabled
- Fixed a bug where WebKit's autocomplete styles made inputs looks broken
- Fixed a bug where aria labels were wrong in `<onex-select>`
- Fixed a bug where clicking the label wouldn't focus the control in `<onex-select>`

## 2.0.0-beta.14

- Added dark theme
- Added `--onex-panel-background-color` and `--onex-panel-border-color` tokens
- Added `--tabs-border-color` custom property to `<onex-tab-group>`
- Added `--track-color` custom property to `<onex-range>`
- Added `tag` part to `<onex-select>`
...
- Fixed a bug where the text color was being erroneously set in `<onex-progress-ring>`
- Fixed a bug where `<onex-progress-bar>` used the wrong part name internally for `indicator`
- Removed background color from `<onex-menu>`
- Updated to Stencil 1.17.3

## 2.0.0-beta.13

- Added `slactivate` and `sldeactivate` events to `<onex-menu-item>`
- Added experimental `<onex-animation>` component
...
- Moved scrolling logic from `<onex-menu>` to `<onex-dropdown>`
...
- Fixed a bug where focus visible observer wasn't removed from `<onex-details>`
...
- Replaced make-shift icon buttons with `<onex-icon-button>` in alert, dialog, drawer, and tag
...
- Fixed a bug where nested form controls were ignored in `<onex-form>`
