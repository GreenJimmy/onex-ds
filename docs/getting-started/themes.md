...
All themes are scoped to classes using the `onex-theme-{name}` convention, where `{name}` is a lowercase, hyphen-delimited value representing the name of the theme. The included light and dark themes use `onex-theme-light` and `onex-theme-dark`, respectively. A custom theme called "Purple Power", for example, would use a class called `onex-theme-purple-power`
...
To activate the theme, apply the `onex-theme-dark` class to the `<html>` element.
